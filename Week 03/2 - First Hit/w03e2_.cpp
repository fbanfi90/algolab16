...
int main()
{
    ...
    while (true)
    {
        ...
        if (hit)
            cout << (long)f2d(p.x()) << " " << (long)f2d(p.y()) << endl;
        else
            cout << "no" << endl;
    }
...
int main_naive()
{
    ...
    while (true)
    {
        ...
        // Find closest intersection point p with an obstacle segment.
        P p;
        bool hit = false;
        for (auto o : os)
        {
            if (do_intersect(r, o))
            {
                ...
                // Update closest intersection.
                if (!hit || squared_distance(r.source(), p_) < squared_distance(r.source(), p))
                    p = p_;
                hit = true;
            }
        }
